...
class ExhibitionCurator:
    def __init__(self, api_key, metadata, embedding_model = SentenceTransformer('all-MiniLM-L6-v2')):
        ...
        checkpoint = time()
        if not os.path.isfile('./Recommend/data/description_embeddings.npy'):
            self.description_embeddings = self.embedding_model.encode(self.descriptions)
            self.description_embeddings = torch.tensor(self.description_embeddings).to('cpu')
            torch.save(self.description_embeddings, ('./Recommend/data/description_embeddings.pt'))
        else:
            self.description_embeddings = torch.load('./Recommend/data/description_embeddings.pt')
    ...
    def get_exhibitions(self, recommendations):
        ...
        recommendation_df = recommendation_df[['image_id','title', 'artist_display', 'cluster_label']]
        exhibitions = []
        # clusters = []
        for cluster_id in recommendation_df['cluster_label'].unique():
            ...
            for index, row in cluster.iterrows():
                exhibition += row['image_id'] + " | " + row['title'] + " | " + row['artist_display'] + "; "
    ...
    def curate(self, recommendations):
        responses = []
        exhibitions = self.get_exhibitions(recommendations)
        ...
        for index, response in enumerate(responses):
            ...
            for line in result:
                ...
                if 'art_pieces' not in exhibition_temp:
                    exhibition_temp['art_pieces'] = exhibition_temp['art_pieces'] = artworks[:-1]
