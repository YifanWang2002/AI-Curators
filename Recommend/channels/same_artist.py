...
class SameArtistChannel:
    ...
    def update_data(self, unique_log, num_artist, interacted_set):
        ...
        artist_ids = [artwork['artist_id'] for artwork in artworks_data]
        ...
        self.artist_ids = [f"Artist: {artist}" for artist in artist_ids]
    ...
    def __call__(self, recommended_set):
        ...
        artist_recs_list = [
            x for sublist in self.candidates_list for x in sublist if x not in exclude_set
        ]
        artist_names = []
        for i, sublist in enumerate(self.candidates_list):
            artist_name = self.artist_ids[i]
            artist_names.extend([artist_name] * len([x for x in sublist if x not in exclude_set]))
