...
class Recommender:
    ...
    def recommend(self, user_id):
        ...
        if self.is_new:
            ...
            top_k_df = self.cold_start_channel()
            if self.configs["as_exhibition"]:
                for cluster_id in top_k_df['cluster_label'].unique():
                    indices = top_k_df[top_k_df['cluster_label'] == cluster_id]['index'].values
                    result = self.metadata.iloc[indices]
                    filename = f"Exhibition {cluster_id+1}"
                    result.to_csv(os.path.join(recommendation_output, filename + ".csv"))
                    save_images(os.path.join(recommendation_output, filename + ".jpg"), result["image_id"])
            else:
                indices = top_k_df['index'].values
                result = self.metadata.iloc[indices]
                if len(result) > 0:
